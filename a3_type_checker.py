...
assert isinstance(result, int), \
       '''poetry_functions.count_vowel_phonemes should return an int, but returned {0}.''' \
       .format(type(result))

# Type check poetry_functions.last_phonemes
result = poetry_functions.last_phonemes(['G', 'AE1', 'P'])
assert isinstance(result, list), \
       '''poetry_functions.last_phonemes should return a list, but returned {0}.''' \
       .format(type(result))
for item in result:
    assert isinstance(item, str), \
...
# Type check poetry_functions.check_vowel_phoneme_counts
result = poetry_functions.check_vowel_phoneme_counts(poem_lines, pattern, word_to_phonemes)
assert isinstance(result, list), \
       '''poetry_functions.check_vowel_phoneme_counts should return a list, but returned {0}.''' \
       .format(type(result))
for item in result:
    assert isinstance(item, str), \
       '''poetry_functions.check_vowel_phoneme_counts should return a list of str,
        but returned a list of {0}.''' \
       .format(type(item))
