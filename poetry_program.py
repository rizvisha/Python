...
def check_poem(poem_lines, pattern, word_to_phonemes, form_name):
    ...
    of vowel phonemes and report the lines that don't; also check whether the 
    ...
    if len(poem_lines) != len(pattern[0]):
        ...
        problem_lines = poetry_functions.check_vowel_phoneme_counts(
            poem_lines, pattern, word_to_phonemes)

        if len(problem_lines) == 0:
            print('\nThe poem has the right number of vowel phonemes '
                  'on each line.\n')
        else:
            print('\n== The poem is not a {}. These lines don\'t have the '
                  'right number of vowel phonemes: == '.format(form_name))
            print('\n'.join(problem_lines) + '\n')
